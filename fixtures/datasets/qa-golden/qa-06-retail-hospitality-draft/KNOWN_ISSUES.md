...
- ❌ **Leave balances literal `"fixture-cumulative"` strings** in `payroll/leave_balances_expected.csv`. Either compute the cumulative balance per pay row from the accrual rates or remove the columns. Today this file proves nothing the engine could be measured against.
