-- Migration: Add RRULE (RFC 5545) support to recurring_schedule
-- Replaces the hardcoded frequency enum with a standards-compliant recurrence rule string.
...
COMMENT ON COLUMN recurring_schedule.frequency IS 'Human-readable frequency label derived from rrule (e.g. weekly, monthly, custom).';
